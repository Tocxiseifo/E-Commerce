...
export default function ProductCard() {
    ...
    return(
        <section className="w-full h-full  bg-white justify-center  ">
                <motion.div initial={{opacity:0 , y:100}} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{duration:1 , ease:'easeOut' ,delay:0.2}} className="flex flex-col items-center  ">
                    <motion.h1 initial={{opacity:0 , x:100}} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{duration:1 , ease:'easeOut' ,delay:0.4}}  className="text-3xl font-bold text-text-color">New Arrival</motion.h1>
                    <motion.p initial={{opacity:0 , x:100}} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{duration:1 , ease:'easeOut' ,delay:0.7}}  className="text-subtitles">Discover our New Arrival products</motion.p>
                </motion.div>
            <div className="flex flex-row flex-wrap  justify-center items-center gap-17">
                {products.slice(5,20).map((product,index) =>{
                    return(
                        <motion.div initial={{opacity:0 , x:-100}} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0 }} transition={{duration:1 , ease:'easeOut' ,delay:index * 0.2}} key={product.id} className="flex  flex-col h-150 w-60 text-left group  mt-40 ">
                            ...
                            <div className="flex flex-col gap-3">
                                <h3 className="text-sm h-12 w-65 text-text-color">
                                    {product.title}
                                </h3>
